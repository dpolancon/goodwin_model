...
# - Fixes boolean parsing robustly
# - Builds hopf_by_cand with maxIm_root_med (if available)
# - Creates simulation plans around Hopf roots: rF = root +/- eps
# - Runs ODE simulations (requires your model ODE function + params)
# - Computes simple cycle diagnostics (amplitude + peak count)
# - Exports: plan CSV, sim metrics CSV, shortlist CSV, plots, LaTeX table, manifest
# ============================================================
...
# ----------------------------
# Robust boolean coercion (fixes your all-FALSE issue)
# ----------------------------
to_bool <- function(x) {
  # Accept logical, numeric, character "TRUE"/"FALSE"
  if (is.logical(x)) return(ifelse(is.na(x), NA, x))
  if (is.numeric(x)) return(ifelse(is.na(x), NA, x != 0))
  if (is.character(x)) {
    y <- tolower(trimws(x))
    return(ifelse(y %in% c("true","t","1"), TRUE,
...
# Hopf verified rows (keep both schema styles)
hopf_verified <- hopf %>%
  mutate(
    hopf_ok = to_bool(hopf_ok),
    rF_root = suppressWarnings(as.numeric(rF_root)),
    rF0 = dplyr::case_when(
      "rF0" %in% names(.) ~ suppressWarnings(as.numeric(rF0)),
      "rF"  %in% names(.) ~ suppressWarnings(as.numeric(rF)),
      TRUE ~ NA_real_
    )
  ) %>%
  filter(is.finite(rF_root), is.finite(rF0)) %>%
  filter(isTRUE(hopf_ok) | reason == "ok")

# ----------------------------
# hopf_by_cand with maxIm_root_med if maxIm_root exists
# ----------------------------
hopf_by_cand <- hopf_verified %>%
  group_by(try_id, cand_id, rF0) %>%
  summarise(
...
cat("Stage 5 input counts:\n")
cat("  stage3_ok rows      =", nrow(stage3_ok), "\n")
cat("  hopf_by_cand rows    =", nrow(hopf_by_cand), "\n")
cat("  merged rows          =", nrow(df), "\n")
cat("  has_hopf rows        =", sum(df$has_hopf, na.rm=TRUE), "\n")
cat("  RH_ok rows           =", sum(df$RH_ok == TRUE, na.rm=TRUE), "\n")
cat("  has_complex rows     =", sum(df$has_complex == TRUE, na.rm=TRUE), "\n")

# ----------------------------
# Stage 5 Plan: pick candidates for simulation
# ----------------------------
# knobs
omega_cap_for_sim <- 0.70   # relax: you can tighten later after you actually see cycles
rF_eps            <- 0.005  # perturb around Hopf root
max_per_cell      <- 3      # per (psi,kappa_max) keep at most N candidates

# Candidate pool: must have hopf, and preferably RH_ok (but don't hard-gate if it kills you)
cand_pool <- df %>%
  filter(has_hopf) %>%
  filter(is.finite(rF_root_med)) %>%
  filter(is.finite(omega_star), omega_star <= omega_cap_for_sim) %>%
  # prefer RH_ok + complex, but keep if NA
  mutate(
    pref = (RH_ok == TRUE) + (has_complex == TRUE) + (hopf_complex == TRUE)
  ) %>%
  arrange(desc(pref), abs(rF - rF_root_med)) %>%
  group_by(psi, kappa_max) %>%
  slice_head(n = max_per_cell) %>%
  ungroup()

# Build simulation plan: rF below/above boundary
sim_plan <- cand_pool %>%
  transmute(
    try_id, cand_id,
    sigma, g_n, i, delta, kappa_min, kappa_max,
    psi, phi2,
    omega_star, d_star,
    lambda_star, f_star, Z_star,
    RH_ok, has_complex,
    n_roots, rF_root_med, rF_root_p25, rF_root_p75,
    rF_sim_low  = rF_root_med - rF_eps,
    rF_sim_high = rF_root_med + rF_eps
  )

write_csv(sim_plan, file.path(dir_stage5, "stage5_sim_plan.csv"))

cat("\nStage 5 plan:\n")
cat("  candidates =", nrow(sim_plan), "\n")
print(sim_plan %>% select(try_id,cand_id,psi,kappa_max,omega_star,rF_root_med,rF_sim_low,rF_sim_high) %>% head(10))

# ----------------------------
# REQUIREMENT: you must already have your model ODE for deSolve
# ------------------------------------------------------------
# This Stage 5 expects you have:
#  - library(deSolve) loaded
#  - an ODE function: goodwin_minsky_ode(t, state, parms) returning list(dstate)
#  - a parameter list builder or 'par_base' plus the ability to override sigma,g_n,i,delta,kappa_max,psi,phi2,rF
#  - initial state: c(e=?, omega=?, d=?)
#
# If those exist, set:
#   ode_fun_name <- "goodwin_minsky_ode"
# and this will run. Otherwise it will just export plan + plots + tables.
# ----------------------------
have_deSolve <- requireNamespace("deSolve", quietly = TRUE)

ode_fun_name <- "goodwin_minsky_ode"
have_ode_fun <- exists(ode_fun_name, mode = "function")

# Simulation settings
t_end   <- 800
dt      <- 0.05
burnin  <- 300   # discard first part for cycle metrics
state0  <- c(e = 0.75, omega = 0.62, d = 0.2)  # EDIT to your baseline init or candidate-based init

# Metric helper (no fancy packages)
count_peaks <- function(x) {
  # counts strict local maxima
  if (length(x) < 3) return(0L)
  sum(x[2:(length(x)-1)] > x[1:(length(x)-2)] & x[2:(length(x)-1)] > x[3:length(x)], na.rm=TRUE)
}

sim_results <- list()

if (have_deSolve && have_ode_fun && exists("par_base")) {
  
  cat("\nRunning simulations (Stage 5)...\n")
  
  ode_fun <- get(ode_fun_name)
  
  times <- seq(0, t_end, by = dt)
  
  for (k in seq_len(nrow(sim_plan))) {
    
    row <- sim_plan[k, ]
    
    # build parameter list for this candidate
    p <- par_base
    p$kappa_min <- row$kappa_min
    p$kappa_max <- row$kappa_max
    p$psi       <- row$psi
    p$phi2      <- row$phi2
    # if you store sigma,g_n,i,delta in params used by ode, assign them too
    p$sigma <- row$sigma
    p$g_n   <- row$g_n
    p$i     <- row$i
    p$delta <- row$delta
    ...
    for (side in c("low","high")) {
      ...
      rF_use <- if (side == "low") row$rF_sim_low else row$rF_sim_high
      p$rF <- rF_use
      
      out <- tryCatch(
        as.data.frame(deSolve::ode(y = state0, times = times, func = ode_fun, parms = p, method = "rk4")),
        error = function(e) NULL
      )
      if (is.null(out)) next
      
      # burn-in subset
      out2 <- out[out$time >= burnin, , drop=FALSE]
      if (nrow(out2) < 10) next
      
      # cycle metrics (simple, robust)
      amp_e     <- diff(range(out2$e, na.rm=TRUE))
      amp_omega <- diff(range(out2$omega, na.rm=TRUE))
      amp_d     <- diff(range(out2$d, na.rm=TRUE))
      ...
      sim_results[[length(sim_results)+1]] <- tibble(
        try_id = row$try_id, cand_id = row$cand_id,
        side = side,
        psi = row$psi, kappa_max = row$kappa_max,
        omega_star = row$omega_star,
        rF = rF_use,
        rF_root_med = row$rF_root_med,
        amp_e = amp_e,
        amp_omega = amp_omega,
        amp_d = amp_d,
        peaks_omega = peaks_omega
      )
      
      # Save a couple of plots per sim
      tag <- paste0("try",row$try_id,"_cand",row$cand_id,"_",side)
      ...
      p_ts <- out2 %>%
        ...
        labs(title = paste("Time series (burn-in removed):", tag), x="time", y="") +
        theme_minimal()
      ggsave(file.path(dir_stage5, paste0("ts_", tag, ".png")), p_ts, width=10, height=8, dpi=160)
      
      p_phase <- ggplot(out2, aes(x=e, y=omega)) +
        geom_path(alpha=0.8) +
        labs(title = paste("Phase plot (e vs omega):", tag), x="e", y="omega") +
        theme_minimal()
      ggsave(file.path(dir_stage5, paste0("phase_e_omega_", tag, ".png")), p_phase, width=7, height=6, dpi=160)
    }
  }
  
} else {
  cat("\nNOTE: Simulations not run.\n")
  cat("  deSolve available?  ", have_deSolve, "\n")
  cat("  ODE function exists?", have_ode_fun, " (expected name: ", ode_fun_name, ")\n", sep="")
  cat("  par_base exists?    ", exists("par_base"), "\n")
  cat("Stage 5 exported plan + will still export tables/plots below.\n")
}

sim_metrics <- if (length(sim_results) > 0) bind_rows(sim_results) else tibble()
write_csv(sim_metrics, file.path(dir_stage5, "stage5_sim_metrics.csv"))

# ----------------------------
# Shortlist from simulations (if ran): look for sustained oscillations
# ----------------------------
short_sim <- sim_metrics %>%
  group_by(try_id, cand_id, psi, kappa_max) %>%
  summarise(
    n_sides = n(),
    peaks_med = median(peaks_omega, na.rm=TRUE),
    amp_omega_med = median(amp_omega, na.rm=TRUE),
    amp_e_med = median(amp_e, na.rm=TRUE),
    amp_d_med = median(amp_d, na.rm=TRUE),
    .groups="drop"
  ) %>%
  arrange(desc(peaks_med), desc(amp_omega_med))

write_csv(short_sim, file.path(dir_stage5, "stage5_shortlist_from_sims.csv"))

# Summary plot if sims exist
if (nrow(sim_metrics) > 0) {
  pM <- ggplot(sim_metrics, aes(x=amp_omega, y=peaks_omega)) +
    geom_point(alpha=0.8) +
    facet_grid(psi ~ kappa_max) +
    labs(title="Stage 5: cycle diagnostics (amp_omega vs peaks_omega)", x="amp(omega)", y="#peaks(omega) after burn-in") +
    theme_minimal()
  ggsave(file.path(dir_stage5, "cycle_diag_scatter.png"), pM, width=12, height=8, dpi=160)
}

# ----------------------------
# LaTeX shortlist table (simple booktabs)
# Requires: \usepackage{booktabs} and if you use [H], \usepackage{float}
# ----------------------------
short_tex <- sim_plan %>%
  select(try_id, cand_id, psi, kappa_max, sigma, omega_star, rF_root_med, rF_sim_low, rF_sim_high) %>%
...
tex_lines <- c(
  "\\begin{table}[H]",
  "\\centering",
  "\\caption{Stage 5 simulation plan (Hopf-centered).}",
  "\\label{tab:stage5_sim_plan}",
  paste0("\\begin{tabular}{", align, "}"),
  "\\toprule",
  paste0(header, " \\\\"),
  "\\midrule",
...
writeLines(tex_lines, file.path(dir_stage5, "stage5_sim_plan.tex"))

# ----------------------------
# Manifest
# ----------------------------
manifest5 <- tibble(
  path = list.files(dir_stage5, recursive = TRUE, full.names = TRUE)
) %>%
  mutate(bytes = file.size(path), ext = tools::file_ext(path)) %>%
  arrange(path)

write_csv(manifest5, file.path(dir_stage5, "manifest_stage5.csv"))

cat("\n====================\nStage 5 outputs (dir_stage5)\n====================\n")
print(manifest5)
cat("\nSaved:\n",
    "  ", file.path(dir_stage5, "stage5_sim_plan.csv"), "\n",
    "  ", file.path(dir_stage5, "stage5_sim_metrics.csv"), "\n",
    "  ", file.path(dir_stage5, "stage5_shortlist_from_sims.csv"), "\n",
    "  ", file.path(dir_stage5, "stage5_sim_plan.tex"), "\n",
    "  ", file.path(dir_stage5, "manifest_stage5.csv"), "\n", sep="")
