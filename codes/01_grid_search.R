...
##  - Rank planes, export FIG A/FIG B Hopf maps with sign(H).
...
steady_state_reduced <- function(par, eps = 1e-12) {
  ...
  ## d* from identity (as in your code)
...
## ============================================================
## 3) Reduced Jacobian (e, omega, d) at SS
## ============================================================
...
score_hopf_plane <- function(df, H_band = 1e-3, tol = 1e-6) {
  dfF <- df %>% dplyr::filter(ok, feasible_RH, is.finite(H))
  if (nrow(dfF) == 0) {
  ...
  # “Hopf band share” is only meaningful if signs exist on both sides
...
rank_planes <- function(OUT_DIR, H_band = 1e-3, tol = 1e-6) {
  csv_files <- fs::dir_ls(OUT_DIR, regexp = "\\.csv$")
  bn <- basename(csv_files)
  
  plane_files <- csv_files[grepl("^hopf_plane_", bn) & !grepl("scores", bn)]
  
  out <- lapply(plane_files, function(f) {
    df <- readr::read_csv(f, show_col_types = FALSE)
    nm <- tools::file_path_sans_ext(basename(f))
    sc <- score_hopf_plane_v2(df, H_band = H_band, tol = tol) %>% dplyr::mutate(plane = nm)
    sc
  }) %>% dplyr::bind_rows()
  
  out %>%
    dplyr::arrange(
      dplyr::desc(has_robust_sign_change),
      dplyr::asc(degeneracy_flag),
      dplyr::desc(feasible_share),
      dplyr::desc(sd_H)
    )
}
...
compare_planes <- function(par_base, planes, ranges, n = 80,
                           H_band = 1e-3, calibrate_varsigma0 = TRUE) {
  ...
  for (j in seq_along(planes)) {
    ...
    sc <- score_hopf_plane(df, H_band = H_band) %>% mutate(p1 = p1, p2 = p2)
    out[[j]] <- sc
  }
...
make_plane_decision_outputs <- function(scores, out_dir = OUT_DIR) {
  plane_tbl <- scores %>%
    mutate(
      plane = paste0(p1, " × ", p2),
      score = 100*feasible_share + 25*hopf_band_share + 10*as.numeric(sign_change)
    ) %>%
    arrange(desc(sign_change), desc(feasible_share), desc(hopf_band_share)) %>%
    select(plane, p1, p2, sign_change, feasible_share, hopf_band_share, n_feasible, n_ok, score)
  
  write.csv(plane_tbl, fs::path(out_dir, "TABLE_plane_ranking_tidy.csv"), row.names = FALSE)
  
  p_sel <- plane_tbl %>%
    mutate(sign_change = as.factor(sign_change)) %>%
    ggplot(aes(x = feasible_share, y = hopf_band_share, label = plane, shape = sign_change)) +
    geom_point(size = 2.7) +
    geom_text(check_overlap = TRUE, nudge_y = 0.002, size = 3) +
    labs(
      title = "Plane selection map: feasibility vs near-Hopf thickness",
      subtitle = "Prefer sign_change=TRUE + high feasible_share. hopf_band_share is a 'near-boundary density' proxy.",
...
make_scenario_outputs <- function(scen_tbl, out_dir = OUT_DIR) {
  ...
  write.csv(scen_tbl2, fs::path(out_dir, "TABLE_scenarios_tidy.csv"), row.names = FALSE)
...
## ============================================================
## 9) DRIVER: baseline + planes + figures + scenarios + summaries
## ============================================================
...
## ============================================================
## Deliverable 1: rank planes + decision plot
## ============================================================
...
## ============================================================
## Deliverable 3: FIG B (use top-ranked plane, but avoid duplicating A)
## ============================================================
top <- scores %>% slice(1)
p1B <- top$p1
p2B <- top$p2
...
cat("\nDONE. Exports in:\n", OUT_DIR, "\n\nKey files:\n",
    ...
    "- scenario_phase_end_* (start/end labeled)\n")
